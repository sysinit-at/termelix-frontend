...
const STORAGE_KEY = "termelix_auth";
...
export function Auth({ onLogin }: AuthProps) {
  ...
  useEffect(() => {
    const checkElectron = async () => {
      ...
      if (isElectron()) {
        const forceShow = localStorage.getItem("termelix_show_server_config");
        if (forceShow === "true") {
          localStorage.removeItem("termelix_show_server_config");
          try {
  ...
  useEffect(() => {
    ...
    if (success) {
      if (isInMobileWebView()) {
        ...
        // (termelix-mobile:-origin callbacks include one), otherwise read it back
