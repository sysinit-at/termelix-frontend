...
export type SessionLogRecord = {
  ...
  // "ssh" is all this build records, but the server passes the stored column through
  // unfiltered, so legacy rows still arrive as "rdp"/"vnc"/"telnet".
  protocol: "ssh" | "rdp" | "vnc" | "telnet";
  // "guacamole" is retained for legacy recordings captured before the RDP/VNC/Telnet
  // feature was removed — those rows still exist on disk and must stay recognisable.
