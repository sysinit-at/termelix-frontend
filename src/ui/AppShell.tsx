...
import { TmuxSessionsColumn } from "@/features/tmux-monitor/TmuxSessionsColumn";
import { isKnownTabType } from "@/shell/known-tab-types";
...
import { ErrorReportingOptInDialog } from "@/user/ErrorReportingOptInDialog.tsx";
...
import { persistedHostId } from "@/shell/persisted-host-id";
...
export function AppShell({
  ...
  const [splitMode, setSplitMode] = useState<SplitMode>(
    () => (localStorage.getItem("termelix_splitMode") as SplitMode) ?? "none",
  );
  const [paneTabIds, setPaneTabIds] = useState<(string | null)[]>(
    () =>
      JSON.parse(localStorage.getItem("termelix_paneTabIds") ?? "null") ??
      Array(6).fill(null),
  );
  useEffect(() => {
  ...
  const [showErrorReportingPrompt, setShowErrorReportingPrompt] =
    useState(false);
  ...
  const [sidebarWidth, setSidebarWidth] = useState(() => {
    const saved = localStorage.getItem("termelix_sidebarWidth");
    return saved ? parseInt(saved, 10) : 291;
  });
  const [sidebarDragging, setSidebarDragging] = useState(false);
  // Which hosts the sessions column is scoped to. Empty means every host — that is the default
  // view, not an empty one.
  const [tmuxHostFilter, setTmuxHostFilter] = useState<Set<string>>(
    () => new Set(),
  );
  const [sessionsColumnWidth, setSessionsColumnWidth] = useState(() => {
    const saved = localStorage.getItem("termelix_sessionsColumnWidth");
  ...
  useEffect(() => {
    localStorage.setItem("termelix_sidebarWidth", String(sidebarWidth));
  }, [sidebarWidth]);

  useEffect(() => {
    localStorage.setItem(
      "termelix_sessionsColumnWidth",
      String(sessionsColumnWidth),
    );
  }, [sessionsColumnWidth]);

  // A selection is a set of the previous account's host ids. The column filters ids it does not
  // recognise, so this is tidiness rather than a leak — but leaving one account's selection
  // sitting in another's shell is the kind of state that becomes a leak later.
  useEffect(() => {
    setTmuxHostFilter(new Set());
  }, [userId]);

  useEffect(() => {
    localStorage.setItem("termelix_splitMode", splitMode);
  }, [splitMode]);

  useEffect(() => {
    localStorage.setItem("termelix_paneTabIds", JSON.stringify(paneTabIds));
  }, [paneTabIds]);
  ...
  // Extracted so the sessions column's retry has something to call. A failed identity request
  // used to be invisible: `isAdmin` quietly became false and `userId` stayed null forever, which
  // anything waiting on the user could only experience as never arriving.
  const loadIdentity = useCallback(() => {
    ...
    getUserInfo()
      .then((info) => {
        ...
        setShowErrorReportingPrompt(!!info.prompt_error_reporting);
      })
      .catch(() => {
        setIsAdmin(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener("termelix:logout", handle);
    return () => window.removeEventListener("termelix:logout", handle);
  }, [onLogout]);
  ...
  /**
   * The server-push channel.
   *
   * Everything else in this app is pull, so a state change reached the operator only when the
   * browser next happened to ask. Two things arrive here that a poll cannot deliver in time:
   *
   *   * `tmux_transition` — a pane going idle is the moment somebody is waiting for, and it is
   *     worth nothing an interval later;
   *   * `revoked` — access has been withdrawn, and by the time it arrives the server has already
   *     torn down every session, tunnel and connection this user held. This stream is the one
   *     survivor, which makes ending the local session here a requirement rather than a courtesy.
   *
   * A subscription that fails changes nothing: every existing poll stays exactly as it was.
   */
  // Held in a ref so the effect below can depend on nothing. A stream is a HELD CONNECTION and
  // the server allows only 8 per user, reclaiming abandoned ones on its heartbeat — so an effect
  // that re-subscribes whenever a callback identity changes churns through that budget and can
  // lock the user out of their own push channel for as long as the reclaim takes.
  const onLogoutRef = useRef(onLogout);
  onLogoutRef.current = onLogout;

  useEffect(() => {
    const subscription = subscribeToEvents({
      onTmuxTransition: (event) => {
        window.dispatchEvent(
          new CustomEvent("termelix:tmux-transition", { detail: event }),
        );
      },
      onTmuxState: (event) => {
        window.dispatchEvent(
          new CustomEvent("termelix:tmux-state", { detail: event }),
        );
      },
      onRevoked: () => onLogoutRef.current(),
    });
    ...
    getUserPreferences()
      .then((prefs) => {
        ...
        if (prefs.storageMode === "cloud") {
          ...
          if (!localStorage.getItem("termelix-local-snapshot")) {
            const SNAPSHOT_KEYS = [
              "termelix-accent",
              "termelix-font-size",
              "i18nextLng",
            ...
            const snap: Record<string, string | null> = {
              __theme: localStorage.getItem("termelix-theme"),
            };
            for (const key of SNAPSHOT_KEYS)
              snap[key] = localStorage.getItem(key);
            localStorage.setItem(
              "termelix-local-snapshot",
              JSON.stringify(snap),
            );
          }
  ...
  useEffect(() => {
    ...
    window.addEventListener("termelix:hosts-changed", onHostsChanged);
    window.addEventListener("ssh-hosts:changed", onHostsChanged);
    window.addEventListener("hosts:refresh", onHostsChanged);
    return () => {
      window.removeEventListener("termelix:hosts-changed", onHostsChanged);
  ...
  useEffect(() => {
    ...
    window.addEventListener("termelix:open-tab", handle);
    return () => window.removeEventListener("termelix:open-tab", handle);
  }, [allHosts]);

  const PERSISTENT_TAB_TYPES: TabType[] = ["terminal", "files", "tunnel"];
        ...
        if (userPrefs.reopenTabsOnLogin) {
          ...
          if (!hasPersistentTabs) {
            ...
            for (const saved of savedTabs as OpenTabRecord[]) {
              // Drop tabs whose type was removed since they were persisted.
              if (!isKnownTabType(saved.tabType)) continue;
          ...
          // minus any whose type was removed since they were persisted.
          setBackgroundTabRecords(
            (savedTabs as OpenTabRecord[]).filter((r) =>
              isKnownTabType(r.tabType),
            ),
          );
        }
      } catch {
  ...
  const openTab = useCallback(function openTab(
    ...
    const tmuxSession = restore?.tmuxSession;
  ...
  /**
   * A click on a host row scopes the sessions column to it.
   *
   * Plain click replaces the selection; shift (or ctrl/cmd, which is the same gesture on a Mac)
   * adds and removes, so several hosts can be watched at once. Clicking the only selected host
   * again clears the scope — the same thing the "Show all" button does, within reach of where the
   * pointer already is.
   */
  const selectHostForSessions = useCallback(
    (host: Host, event: React.MouseEvent) => {
      const extend = event.shiftKey || event.metaKey || event.ctrlKey;
      setTmuxHostFilter((prev) => {
        if (extend) {
          const next = new Set(prev);
          if (!next.delete(host.id)) next.add(host.id);
          return next;
        }
        if (prev.size === 1 && prev.has(host.id)) return new Set();
        return new Set([host.id]);
      });
    },
    [],
  );
  ...
  function connectHost(host: Host, preferredType?: TabType) {
    const type = preferredType ?? "terminal";
  ...
  const SESSION_TAB_TYPES: TabType[] = ["terminal", "serial"];
  ...
  const onSessionsColumnMouseDown = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
      const startX = e.clientX;
      const startW = sessionsColumnWidth;
      function onMove(ev: MouseEvent) {
        setSessionsColumnWidth(
          Math.max(200, Math.min(520, startW + ev.clientX - startX)),
        );
      }
      function onUp() {
        window.removeEventListener("mousemove", onMove);
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        >
          <HostsPanel
            ...
            focusedHostIds={tmuxHostFilter}
            onSelectHost={selectHostForSessions}
          />
        </div>
  ...
  const sidebarHeader = (
    <div className="flex flex-row items-center border-b border-border h-12.5 shrink-0">
      <span className="flex-1 text-sm font-semibold text-foreground px-3">
  ...
  return (
    <ServerStatusProvider isAuthenticated={!!username}>
      <div className="flex w-screen bg-background" style={{ height: "100dvh" }}>
        ...
        {/* Sessions column — permanent, not a view you open. Scoped by the host list's
            selection; empty selection means every host. */}
        <TmuxSessionsColumn
          hosts={allHosts}
          selectedHostIds={tmuxHostFilter}
          onShowAll={() => setTmuxHostFilter(new Set())}
          onAttach={(host, sessionName) =>
            openTab(host, "terminal", {
              instanceId:
                typeof crypto.randomUUID === "function"
                  ...
                  : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`,
              restoredSessionId: null,
              savedLabel: sessionName,
              tmuxSession: sessionName,
            })
          }
          width={sessionsColumnWidth}
          onResizeStart={onSessionsColumnMouseDown}
          userId={userId}
          readiness={
            identityFailed
              ? "unavailable"
      ...
      <ErrorReportingOptInDialog
        open={showErrorReportingPrompt}
        onClose={() => setShowErrorReportingPrompt(false)}
      />
