  ...
  const handleSave = async () => {
    ...
    try {
      const data = {
        ...
        // The "existing_password" sentinel means "unchanged" — omit it so the server
        // keeps the stored secret (write-only: the real value was never sent to us).
        password:
          credForm.password === "existing_password"
            ? undefined
      ...
      if (!adminTargetUserId) {
        window.dispatchEvent(new CustomEvent("termelix:credentials-changed"));
      }
      onSave(saved);
    } catch (err) {
