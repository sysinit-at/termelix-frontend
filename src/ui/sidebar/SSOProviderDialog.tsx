...
export function SSOProviderDialog({
  ...
  // The server no longer returns the stored secret (write-only secrets), only whether one is
  // set. Without this the field is indistinguishable from "never configured".
  ...
  useEffect(() => {
    ...
    if (provider) {
      ...
      setSecretIsSet(
        Boolean(
          (provider as { hasClientSecret?: boolean }).hasClientSecret ??
          (provider as { hasBindPassword?: boolean }).hasBindPassword,
        ),
      );
...
function OIDCConfigFields({
  ...
  const docsHref = simplified
    ? "https://docs.termelix.site/features/authentication/github-google"
    : "https://docs.termelix.site/features/authentication/oidc";
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Field label={t("admin.oidcClientSecret")} required={!secretIsSet}>
        <Input
          type="password"
          value={oidc.client_secret}
          onChange={(e) => setOidcField("client_secret", e.target.value)}
          placeholder={
            secretIsSet ? t("admin.secretSetLeaveBlank") : "your-client-secret"
          }
          className="text-xs"
        />
      </Field>
...
function LDAPConfigFields({
  ...
  return (
    <div className="flex flex-col gap-3">
      <span className="text-[10px] text-muted-foreground">
        <a
          href="https://docs.termelix.site/features/authentication/ldap"
          target="_blank"
          rel="noreferrer"
          className="text-accent-brand hover:underline"
        >
      ...
      <Field label={t("admin.ldapBindPassword")} required={!secretIsSet}>
        <Input
          type="password"
          value={ldap.bindPassword}
          onChange={(e) => setLdapField("bindPassword", e.target.value)}
          placeholder={
            secretIsSet ? t("admin.secretSetLeaveBlank") : "bind password"
          }
          className="text-xs"
        />
      </Field>
