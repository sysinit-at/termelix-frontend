...
import { GlobalTmuxView } from "./GlobalTmuxView";
...
const LS_PREFIX = "termelix-tmux-monitor-";
...
export function TmuxMonitor({
  ...
  // Last successful overview per host. A ref, not state: it is read when a host is selected and
  // must never itself trigger a render.
  const overviewCacheRef = useRef<Map<number, TmuxOverview>>(new Map());
  ...
  useEffect(() => {
    ...
    window.addEventListener("termelix:hosts-changed", handler);
    return () => window.removeEventListener("termelix:hosts-changed", handler);
  }, [loadHosts]);
  ...
  const loadOverview = useCallback(
    async (hostId: number, silent = false) => {
      ...
      try {
        ...
        overviewCacheRef.current.set(hostId, data);
  ...
  useEffect(() => {
    ...
    // Show what we last knew about this host straight away rather than blanking.
    //
    // An overview costs one SSH exec against the remote, which on anything further away than
    // localhost is the difference between instant and a visible wait. Blanking first meant the
    // column emptied on every host switch and stayed empty for that whole round trip, so the
    // fastest path — flicking back to a host looked at moments ago — felt the slowest.
    //
    // The refresh still runs; this only decides what is on screen while it does. `null` for a
    // host never seen keeps the skeleton path intact.
    setOverview(overviewCacheRef.current.get(selectedHostId ?? -1) ?? null);
  ...
  // -- server push ----------------------------------------------------------
  //
  // The poll above stays: this shortens the latency, it does not replace the mechanism. A
  // transition is the moment somebody is actually waiting for — a pane going idle is worth
  // nothing an interval later — so when the server says a pane changed on the host being
  // watched, refresh immediately instead of waiting out the remainder of the poll.
  //
  // Refreshing rather than applying the pushed snapshot directly is deliberate. The overview the
  // UI renders is richer than the event payload, so folding a partial snapshot in would leave
  // fields stale next to fields that just changed, and the two would disagree on screen with no
  // way to tell which was right.
  useEffect(() => {
    if (selectedHostId === null || !isVisible) return;

    const onTransition = (event: Event) => {
      const detail = (event as CustomEvent).detail as { hostId?: number };
      if (detail?.hostId === selectedHostId && !document.hidden) {
        loadOverview(selectedHostId, true);
      }
    };

    window.addEventListener("termelix:tmux-transition", onTransition);
    return () =>
      window.removeEventListener("termelix:tmux-transition", onTransition);
  }, [selectedHostId, isVisible, loadOverview]);
      ...
      <div className="flex h-full w-full flex-col bg-background text-foreground">
        <div className="flex items-center gap-2 border-b border-border px-3 py-2">
          ...
          >
            ...
            {t("tmuxMonitor.perHostView")}
          </Button>
        </div>
        <div className="min-h-0 flex-1">
          <GlobalTmuxView isVisible={isVisible} />
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-border px-3 py-2">
          ...
          <button
            ...
            title={t("tmuxMonitor.allHosts")}
            aria-label={t("tmuxMonitor.allHosts")}
          ...
          <span className="ml-auto flex items-center gap-2">
            ...
            <a
              href="https://docs.termelix.site/features/terminal/tmux"
              target="_blank"
              rel="noreferrer"
              className="text-muted-foreground hover:text-foreground"
        ...
        {/* Radix wraps the viewport content in a display:table div sized to
            ...
            truncate instead. */}
        {/* `min-h-0` is load-bearing, not decoration. A flex child defaults to `min-height: auto`,
            which refuses to shrink below its content — so `flex-1` alone made this grow to fit every
            session instead of being bounded by the rail, and the viewport never got a scrollbar. The
            column simply ran off the bottom of the window with no way to reach the rest. */}
        <ScrollArea className="min-h-0 flex-1 [&_[data-slot=scroll-area-viewport]>div]:!block">
          <div className="p-2">
            {/* The host list had no indicator at all — `hostsLoading` was used only to suppress
                the "no hosts" message, so the column sat blank for the whole fetch and the panel
                looked broken rather than busy. This is the first thing shown when the monitor is
                opened, so it is the wait most likely to be read as "it does not work". */}
            {hostsLoading && (
