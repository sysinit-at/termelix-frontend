...
// Dolphin's own iconography. The extension table lives in the Breeze module and is far wider
// than the eight categories this switch could express, so a .deb, a .sqlite or a .pem now looks
// like itself instead of collapsing into a generic "config" glyph.
const getFileIcon = (file: FileItem, viewMode: "grid" | "list" = "grid") => {
  ...
  // Breeze mimetype icons are full-colour and must not be tinted, or every file flattens into a
  // silhouette. Only the monochrome folder/symlink glyphs still take the type colour.
  const colorClass =
    file.type === "directory" || file.type === "link"
      ? getFileTypeColor(file)
      : "";

  return <Icon className={`${iconClass} ${colorClass}`.trimEnd()} />;
};
