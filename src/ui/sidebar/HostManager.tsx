...
export function HostManager({
  ...
  const [termelixIdLinkedIds] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    ...
    window.addEventListener("termelix:hosts-changed", reloadHosts);
    window.addEventListener("termelix:credentials-changed", reloadCredentials);
    return () => {
      window.removeEventListener("termelix:hosts-changed", reloadHosts);
      window.removeEventListener(
        "termelix:credentials-changed",
        reloadCredentials,
      );
    };
  }, []);
  ...
  const renderEditorView = () => {
    ...
    const showSshSubTabs = isHost && SSH_GROUP_TABS.has(activeHostTab as never);
